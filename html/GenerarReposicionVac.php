...
<body>
	...
	<h2>Generar Reposición de Vacuna</h2>
	<form action="mailto:<?=$_POST['email']?>" method="post" enctype="text/plain">
		<?php foreach( $this->vacunas as $p ) { ?>
			<span><span class="nombres-azulados">Nombre de Vacuna: </span><?= $p['descripcion'] ?></span>
			</br>
		...
		<input type="hidden" name="Nombre_de_Vacuna" value="<?=$p['descripcion']?>">
